...
const EditSession = () => {
  ...
  console.log(queryParams.get("go"));
  ...
  const updateSession = async (values) => {
    ...
    try {
      ...
      const { data } = await axiosInstance.put(
        `${import.meta.env.VITE_BASE_URL}/api/class/updateClass`,
        body,
      );
      console.log(data);
      setloading(false);
      toast.success(data.message);
      if (queryParams && queryParams.get("go") === "dashboard") {
  ...
  const getSpecificSession = async () => {
    try {
      const { data } = await axiosInstance.get(
        `${
          import.meta.env.VITE_BASE_URL
        }/api/class/getSpecificClass/${sessionId}`,
      );
      console.log(data);
      setFieldValue("name", data?.data?.name);
      setFieldValue("purpose", data?.data?.purpose);
      setFieldValue("description", data?.data?.description);
      setFieldValue("instructor", data?.data?.instructor?._id);
      setFieldValue("date", data?.data?.date);
      setFieldValue(
        "startTime",
        moment(data?.data?.startTime, "hh:mm A").toDate(),
      );
      setFieldValue("length", parseInt(data?.data?.length));
                ...
                <Grid item md={6} xs={12}>
                  ...
                  <div className="w-full">
                    <ReactDatePicker
                      ...
                      dateFormat="h:mm aa"
                      className={`border ${
                        touched.startTime && Boolean(errors.date)
                          ? "border-red-500"
                          : "border-gray-300"
